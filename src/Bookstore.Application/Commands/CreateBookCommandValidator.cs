...
public class CreateBookCommandValidator : AbstractValidator<CreateBookCommand>
{
    public CreateBookCommandValidator()
    {
        RuleFor(x => x.Request.Title)
            .NotEmpty().Length(3, 6).WithMessage("Title must be between 3 and 5 characters");
        RuleFor(x => x.Request.Author)
            .NotEmpty().Length(3, 6).WithMessage("Author must be between 3 and 5 characters");
        ...
        RuleFor(x => x.Request.PublishDate)
            .NotEmpty().Must(x => x.StartOfDay() >= DateTime.Now.StartOfDay()).WithMessage("Publish date must be greater than now");
    }
}